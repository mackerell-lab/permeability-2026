...
def calc_regression_metrics(
    ...
    """
    Drop-in replacement for calc_regression_metrics that:
      - removes classification
      - supports nested CV
      - encodes (outer, inner) → single cv_cycle
      - preserves downstream schema
    """

    records = []

    for (outer_f, inner_f, method, split), g in df.groupby(
        [outer_fold_col, inner_fold_col, method_col, split_col]
    ):
        # --- Matrix → linear index ---
        cv_cycle = (outer_f - 1) * n_inner + (inner_f - 1)

        y_true = g[val_col].values
    ...
    metric_df = pd.DataFrame(
        records,
        columns=["cv_cycle", "method", "split", "mae", "mse", "r2", "rho"],
    )
