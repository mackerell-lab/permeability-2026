# VMD FragMap Plugin
#
# Copyright (c) 2016-2024 SilcsBio, LLC. All Rights Reserved.
#
...
namespace eval ::scrolledwindow:: {
    proc scrolledwindow {w args} {
        variable {}

        set canvas [canvas $w.canvas -width 10 -height 10 -highlightt 0]
        ttk::scrollbar $w.scrollx -orient horizontal -command "$canvas xview"
        ttk::scrollbar $w.scrolly -command "$canvas yview"
        ...
        $canvas configure -scrollregion [list 0 0 100 100] \
                          -xscrollcommand "$w.scrollx set" \
                          -yscrollcommand "$w.scrolly set"

        # bind <Configure>
        bind $canvas <Configure> [namespace code [list resize $w]]

        return $w
    }

    proc content {w} {
        return $w.canvas.container.content
    }
    ...
    proc resize {w} {
        ...
        if { [winfo reqwidth $content] > [winfo width $canvas] } {
            grid $w.scrollx  -row 1 -column 0 -sticky ew
        } else {
            grid forget $w.scrollx
        }
        # Vertical scrolling.
        if { [winfo reqheight $content] > [winfo height $canvas] } {
            grid $w.scrolly  -row 0 -column 1 -sticky ns
        } else {
...
namespace eval ::silcs:: {
    ...
    variable mcsilcsdir "mcsilcs"   # SILCS-MC folder
    variable maplevel_lower -2
    variable maplevel_upper 2
    ...
    variable fragmaps_available {
        ...
        mamn {
            label "Positively Charged Map"
            color 10 # cyan
            visible 0
            cutoff -1.2
        }
        acec {
            label "Negatively Charged Map"
            color 3 # orange
            visible 0
            cutoff -1.2
        }
        ...
        argc {
            label "Arginine CZ Map"
            color 10 # cyan
            visible 0
            cutoff 0.075
        }
        argn {
            label "Arginine NH* Map"
            color 10 # cyan
            visible 0
            cutoff 0.075
        }
        asnn {
            label "Asparagine ND2 Map"
            color 0 # blue
            visible 0
            cutoff 0.075
        }
        asno {
            label "Asparagine OD1 Map"
            color 1 # red
            visible 0
            cutoff 0.075
        }
        asp {
            label "Aspartic acid CG Map"
            color 3 # orange
            visible 0
            cutoff 0.075
        }
        cys {
            label "Cysteine SG Map"
            color 4 # yellow
            visible 0
            cutoff 0.075
        }
        glnn {
            label "Glutamine NE2 Map"
            color 0 # blue
            visible 0
            cutoff 0.075
        }
        glno {
            label "Glutamine OE1 Map"
            color 1 # red
            visible 0
            cutoff 0.075
        }
        glu {
            label "Glutamic acid CD Map"
            color 3 # orange
            visible 0
            cutoff 0.075
        }
        hsdn {
            label "Histidine(HSD) NE2 Map"
            color 1 # red
            visible 0
            cutoff 0.075
        }
        hsdn2 {
            label "Histidine(HSD) ND1 Map"
            color 0 # blue
            visible 0
            cutoff 0.075
        }
        hsen {
            label "Histidine(HSE) NE2 Map"
            color 0 # blue
            visible 0
            cutoff 0.075
        }
        hsen2 {
            label "Histidine(HSE) ND1 Map"
            color 1 # red
            visible 0
            cutoff 0.075
        }
        hsp {
            label "Histidine(HSP) NE2/ND1 Map"
            color 0 # blue
            visible 0
            cutoff 0.075
        }
        ile {
            label "Isoleucine C* Map"
            color 7 # green
            visible 0
            cutoff 0.075
        }
        ...
        met {
            label "Methionine C*/SD Map"
            color 7 # green
            visible 0
            cutoff 0.075
        }
        phe {
            label "Phenylalanine C* Map"
            color 7 # green
            visible 0
            cutoff 0.075
        }
        ...
        thr {
            label "Threonine OG1 Map"
            color 5 # tan
            visible 0
            cutoff 0.075
        }
        trpc {
            label "Tryptophan C* Map"
            color 7 # green
            visible 0
            cutoff 0.075
        }
        trpn {
            label "Tryptophan NE1 Map"
            color 0 # blue
            visible 0
            cutoff 0.075
        }
        tyrc {
            label "Tyrosine C* Map"
            color 7 # green
            visible 0
            cutoff 0.075
        }
        tyro {
            label "Tyrosine OH Map"
            color 5 # tan
            visible 0
            cutoff 0.075
        }
    ...
    variable fragmaps_enabled {
        apolar hbdon hbacc mamn acec meoo excl
        benc prpc forn foro dmeo iminh imin tipo
        clbx flbx brbx clex fetx tfec
        ala argc argn asnn asno asp cys glnn glno
        glu hsdn hsdn2 hsen hsen2 hsp ile leu lys
        met phe pro ser thr trpc trpn tyrc tyro val
    }
    variable protmaps {
        ala argc argn asnn asno asp cys glnn glno
        glu hsdn hsdn2 hsen hsen2 hsp ile leu lys
...
SilcsBio LLC, 2024 - https://silcsbio.com" -anchor center -justify center -background #3857a5 -foreground white -font bold
    ...
    set entries {
        ...
        {var "mcsilcs" label "SILCS-MC folder:" tv ::silcs::mcsilcsdir}
    }
    ...
    ::scrolledwindow::scrolledwindow $loc
    set frame [::scrolledwindow::content $loc]
    ...
    for {set i 0} {$i < $nelem} {incr i} {
        ...
        ttk::label $frame.${var}_label -text $label -anchor w
        ttk::entry $frame.$var -textvariable $tv

        grid $frame.${var}_label -column 0 -row $i -sticky nswe
        grid $frame.$var -column 1 -row $i -sticky nswe
    }
    ...
    for {set i 0} {$i < $nelem} {incr i} {
        ...
        ttk::label $frame.${key}_label -text $label -anchor w
        ttk::entry $frame.$key -textvariable ::silcs::$tv -width 50

        grid $frame.${key}_label -column 0 -row $idx -sticky nswe
        grid $frame.$key -column 1 -row $idx -sticky nswe
    }
    ...
    set interior [ttk::frame $vis.interior]

    ttk::label $interior.label -text "You must have at least a molecule and a map loaded." -justify center -anchor center
    pack $interior.label -expand 1 -fill both -padx 4 -pady 4
    grid $interior -column 0 -row 0 -sticky nswe
    grid columnconfigure $interior 0 -weight 1
    ...
    ##$w.hlf.nb add $w.hlf.nb.mcsilcs -text "SILCS-MC"
...
Created by SilcsBio LLC, 2024"
    ...
    #ttk::button $w.hlf.controlButtons.loadmcsilcs -text "Load SILCS-MC" -command { ::silcs::mcsilcs_dialog }
    ttk::button $w.hlf.controlButtons.quit -text "Exit FragMap Tools" -command { ::silcs::quit }
    grid $w.hlf.controlButtons -column 0 -row 3 -sticky ns -pady "0 10"
    ...
    grid $w.hlf.controlButtons.quit -column 2 -row 0 -sticky nswe
...
proc ::silcs::init {} {
    ...
    for {set i 0} {$i < $nelem} {incr i} {
        ...
        if { $key eq "excl" } {
            ...
            if { $key in $protmaps } {
                set ::silcs::$tv [file join "{mapdir}" [format "{prefix}.%s.prob.map" $key]]
            } else {
...
proc ::silcs::show_fragmap {} {
    ...
    variable w
    set vis $w.hlf.nb.vis
    set frame [::scrolledwindow::content $vis]
    set topmol [molinfo top]
...
proc ::silcs::refresh_fragmap {} {
    variable w
    set vis $w.hlf.nb.vis
    destroy $vis.interior

    set interior [ttk::frame $vis.interior]
    grid $interior -column 0 -row 0 -sticky nwse -padx 10 -pady 5
    grid columnconfigure $vis 0 -weight 1
    grid rowconfigure $vis 0 -weight 1

    ::scrolledwindow::scrolledwindow $interior
    set frame [::scrolledwindow::content $interior]

    grid $frame -column 0 -row 0 -sticky nwse -padx 10 -pady 5
    grid columnconfigure $interior 0 -weight 1
    grid rowconfigure $interior 0 -weight 1

    # decoration
    ttk::separator $interior.sep -orient horizontal
    grid $interior.sep -column 0 -row 1 -sticky we -pady 4

    # protein surface panel
    set surface_frame [ttk::frame $interior.surface]
    grid $surface_frame -column 0 -row 2 -sticky we -padx 10 -ipady 5

    # display protein surface map options
    ttk::label $surface_frame.header_surftype -text "Surface Type" -anchor center
    ttk::label $surface_frame.header_inputmol -text "Molecule" -anchor center
    ...
    ttk::checkbutton $surface_frame.surf_visible -variable ::silcs::surf_visible -onvalue 1 -offvalue 0 -command "::silcs::loadsurface surf"
    ttk::label $surface_frame.surf_label -text "Protein Surface"
    ttk::combobox $surface_frame.surf_mollist -values $inputmol -state readonly
    if {[llength $inputmol] && [$surface_frame.surf_mollist current] < 0} { $surface_frame.surf_mollist current 0 }

    ttk::checkbutton $surface_frame.cartoon_visible -variable ::silcs::cartoon_visible -onvalue 1 -offvalue 0 -command "::silcs::loadsurface cartoon"
    ttk::label $surface_frame.cartoon_label -text "Protein Cartoon"
    ttk::combobox $surface_frame.cartoon_mollist -values $inputmol -state readonly
    if {[llength $inputmol] && [$surface_frame.cartoon_mollist current] < 0} { $surface_frame.cartoon_mollist current 0 }

    ttk::checkbutton $surface_frame.excl_visible -variable ::silcs::excl_fragmap_visible -onvalue 1 -offvalue 0 -command "::silcs::loadfragmap excl"
    ttk::label $surface_frame.excl_label -text "Exclusion Map" -anchor w

    set row 1
    grid $surface_frame.header_surftype -row $row -column 1 -sticky nswe
    grid $surface_frame.header_inputmol -row $row -column 2 -sticky nswe

    incr row
    grid $surface_frame.surf_visible -row $row -column 0 -sticky nswe
    grid $surface_frame.surf_label -row $row -column 1 -sticky nswe
    grid $surface_frame.surf_mollist -row $row -column 2 -sticky nswe

    incr row
    grid $surface_frame.cartoon_visible -row $row -column 0 -sticky nswe
    grid $surface_frame.cartoon_label -row $row -column 1 -sticky nswe
    grid $surface_frame.cartoon_mollist -row $row -column 2 -sticky nswe

    incr row
    grid $surface_frame.excl_visible -row $row -column 0 -sticky nswe
    grid $surface_frame.excl_label -row $row -column 1 -sticky nswe

    # fragmap panel

    ttk::label $frame.header_maptype -text "FragMap Type" -anchor center
    ttk::label $frame.header_maplevel -text "GFE Level" -anchor center
    ...
    for {set i 0} {$i < $nelem} {incr i} {
        ...
        if {$key == "excl"} {
            ...
            # change offset if protmaps
            if { $key in $protmaps } {
                set resolution 0.001
                set digit 3
                set lowermaplevel 0.001
                set uppermaplevel 1.000
            } else {
                set resolution 0.1
                set digit 2
                set lowermaplevel $::silcs::maplevel_lower
                set uppermaplevel $::silcs::maplevel_upper
            }
        }

        # add new header if fragmap type is ala
        if { $key == "ala" } {
            ttk::label $frame.header_blanktype -text "-v-v-v-" -anchor center
            ttk::label $frame.header_blanklevel -text "-v-v-v-" -anchor center
            ttk::label $frame.header_probmaptype -text "Protein SC ProbMap Type" -anchor center
            ttk::label $frame.header_probmaplevel -text "Probability Level" -anchor center
            grid $frame.header_blanktype -column 1 -row $row -sticky nswe -padx 10
            grid $frame.header_blanklevel -column 2 -row $row -columnspan 1 -sticky nswe
            set row [expr $row + 1]
            grid $frame.header_probmaptype -column 1 -row $row -sticky nswe -padx 10
            grid $frame.header_probmaplevel -column 2 -row $row -columnspan 1 -sticky nswe
            set row [expr $row + 1]
            }

        ttk::checkbutton $frame.${key}_visible -variable ::silcs::$flag -onvalue 1 -offvalue 0 -command "::silcs::loadfragmap $key"
        ttk::label $frame.${key}_label -text $label -anchor w
        ttk::frame $frame.${key}Frame
        ttk::button $frame.${key}Frame.minus -text "-" -width 2 -command "::silcs::updatelevel $key lower"
        scale $frame.${key}Frame.scale -orient horizontal \
            -showvalue false -from $lowermaplevel -to $uppermaplevel -resolution $resolution -digit $digit \
            -bg #7f7f7f -activebackground #7f7f7f \
            -variable ::silcs::$cutoff \
            -command "::silcs::updatelevel $key"
        ttk::button $frame.${key}Frame.plus -text "+" -width 2 -command "::silcs::updatelevel $key upper"
        ttk::label $frame.${key}_cutoff_label -textvariable ::silcs::$cutoff -anchor e -justify right
        ttk::label $frame.${key}_cutoff_label_unit -text "kcal/mol" -anchor w

        grid $frame.${key}_visible -column 0 -row $row -sticky nwe
        grid $frame.${key}_label -column 1 -row $row -sticky nwe -padx 10
        grid $frame.${key}Frame -column 2 -row $row -sticky nwe -padx 10
        grid $frame.${key}_cutoff_label -column 3 -row $row -sticky nwe -padx 10
        grid $frame.${key}_cutoff_label_unit -column 4 -row $row -sticky nwe

        grid $frame.${key}Frame.minus -column 0 -row 0 -sticky nwse
        grid $frame.${key}Frame.scale -column 1 -row 0 -sticky we
        grid $frame.${key}Frame.plus -column 2 -row 0 -sticky nwse
        grid columnconfigure $frame.${key}Frame 1 -weight 1
    }
    grid $frame.header_maptype -column 1 -row 0 -sticky nswe -padx 10
    grid $frame.header_maplevel -column 2 -row 0 -columnspan 1 -sticky nswe
...
proc ::silcs::loadsurface {surftype args} {
    ...
    set vis $w.hlf.nb.vis
    set frame [::scrolledwindow::content $vis]
    set surface_frame $vis.interior.surface

    set flag [set ::silcs::${surftype}_visible]
    set topmol [lindex [split [$surface_frame.${surftype}_mollist get] ":"] 0]
...
proc ::silcs::updatelevel {maptype level} {
    ...
    set protmaps $::silcs::protmaps
    if { $maptype in $protmaps } {
        set offset 0.001
        # puts stdout "offset = $offset"
    } else {
        set offset 0.1
        # puts stdout "offset = $offset"
    }
    ...
    if {[info exists maphash($maptype)]} {
        ...
        set $cutoffvar [expr [format "%.3f" $level]]
    }
}
...
proc ::silcs::ligand_dialog {} {
    ...
    variable w
    set types {
        {{Ligand Files}    {{.pdb} {.mol2}}}
    }
    set molfiles [tk_getOpenFile -filetypes $types \
        -multiple 1 -title "Load Ligands"]
...
proc ::silcs::refresh_mcsilcs {} {
    ...
    for {set i 0} {$i < $nelem} {incr i} {
        ...
        set totalframe [dict get $::silcs::mcligandhash($key) numframes]
        set lastframe [expr $totalframe - 1]
        scale $mcsilcs.frame.mcmol${i}Frame.scale -orient horizontal \
            -showvalue false -from 0 -to $lastframe -resolution 1 \
...
proc sdfbfactor { fname } {
  ...
  while { [gets $in line] != -1 } {
    ...
    if {$token == ">  <"} {
        if {[string range $line 4 7] == "LGFE"} {
            set lgfe [gets $in line]
        }
        if {[string range $line 4 17] == "atom.dprop.GFE"} {
            set beta [split [gets $in line]]
        }
    }
    if {$token == "$$$$"} {
        $all frame $frame
